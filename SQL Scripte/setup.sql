...
(
 ...
 CONSTRAINT [FK_22] FOREIGN KEY ([Tenant_ID])  REFERENCES [Tenant]([Tenant_ID]) ON DELETE CASCADE,
...
(
 ...
 CONSTRAINT [FK_52] FOREIGN KEY ([Tenant_ID])  REFERENCES [Tenant]([Tenant_ID]) ON DELETE CASCADE,
...
(
 ...
 CONSTRAINT [FK_105] FOREIGN KEY ([Tenant_ID])  REFERENCES [Tenant]([Tenant_ID]) ON DELETE CASCADE
);
GO
...
AS
BEGIN
	select (gesamtbetrag * (select Persons from Tenant where Last_Name = @nachnameMieter)) as Betrag, usage, 'pro Personen' as Abrechnungsart
	from Nebenkosten_P_proPerson
	where Jahr = @jahr
	union
	select (gesamtbetrag * (select f.size from Flat as f join Contract as c on c.Contract_ID = f.Contract_ID join Tenant as t on t.Tenant_ID = c.Tenant_ID where Last_Name = @nachnameMieter)) as Betrag, usage, 'pro Quadratmeter' as Abrechnungsart
